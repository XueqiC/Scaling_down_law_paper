...
\centering
\small
\caption{The table audits QA measurement scope on Gemma-3-1B by reporting conditional loss and $\Delta L$ from dense (native-token nats; negative is improvement), 256 fixed references per set. Distillation labels denote nominal supervised-token budgets. 2Wiki supplies all context; MuSiQue supplies supporting paragraphs; TriviaQA uses the no-context control. All 24 cells were measured. Rows identify compression states; paired columns give loss and change from dense for each benchmark. This is a \mbox{post-hoc} measurement-scope audit of \mbox{pre-specified} states.}
\label{tab:qa_scope}
