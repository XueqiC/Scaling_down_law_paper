...
model. All four references meet the criterion pre-specified for this round. A retrospective ablation on the same candidates (Appendix Table~\ref{tab:s3-ablation}) shows where
that value comes from: a development-fixed heuristic with no numeric prediction, the student first for question answering
and the largest feasible quantization otherwise, matches the rule except on mathematics, where the
development medians the rule uses beat it, 0.004 against 0.016 nats; the source-conditioned relations of Section~\ref{sec:laws}
add no decision value; and against a perfect quantization selection the rule's choice is still
better by 0.09 to 0.89 nats on question answering.
