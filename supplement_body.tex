...
these data do not settle the choice of descriptor.
...
arm also carries a source-free relation for the cases in which the source inputs add nothing.
...
\caption{Paired improvements with the zero line marked; positive values mean that the first-named model is better. (a) Full model against its variant without $D_0$, per arm and capability; intervals are shown only where a cluster bootstrap was computed (leave-one-stage-out panels), and $n$ is the number of evaluated source states. (b) Source-conditioned candidate against the strongest source-free baseline for all twenty registered tests (hollow markers: frozen prospective; filled markers: leave-one-out). (c) Shared power form against the same-input alternatives A1 and A2 on the pruning configuration tests.}
\label{fig:gains}
\end{figure}
...
Pythia panel is reported in Section 7 of the main paper and
Appendix H.
