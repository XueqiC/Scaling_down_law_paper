...
\label{eq:quant2d}
\end{equation}
with one four-vector of state coefficients $\phi(\mathbf x)$ per term (twenty coefficients) and
...
the same development fit, but predicts the unseen bit-width much less
accurately (bit-test MAE 1.30, 1.39, 1.94 against 0.19, 0.21, 0.44), as do
...
parametrization; the development data do not identify any curvature
in bit-width.
...
development points and scored on the 52 test points (nine unseen-pool
trajectories and four held-out student trajectories at the development
pools; these predictions were specified after the frozen matrix), the
second form of Equation 4 of the main paper has the lowest error among
the forms on the 270M unseen-pool trajectories (0.022, 0.020, and 0.316
nats for math, code, and QA against 0.060, 0.047, and 0.862 for the
frozen constant) and is second to the reuse-only form for QA on the 1B
trajectories (0.478 against 0.342). The first form, which has no budget
term, performs poorly for QA on each group (1.15 to 2.26 nats). On the held-out 4B student the frozen
...
(0.404 with the loss descriptor, 0.122 with the size descriptor), and
...
space, and the logit variance). Within a parametric family fitted by ridge regression, the statistics
improve four of nine arm-and-capability pairs (pruning math and QA, grouped-quantization math and QA),
each with an interval above zero; in nine of nine pairs both statistics-augmented linear candidates
...
49.4\%) when the three statistics are added to the inputs.
...
intervals above zero for math and code, whereas adding $D_0$ does not
(Figure 10A of the main paper). The source-conditioned forms of Section 5 of the main paper use the three inputs per capability, and each
arm also carries a source-free relation for the cases in which the source inputs add nothing to
...
six-model, five-density pruning panel (retained density $d$ from $0.9$ to $0.7$), the
...
protocol at pool sizes $U=75$ and $U=450$ traces per domain) were scored on the main-protocol probe
...
not appear on TriviaQA ($+0.1$ to $+0.4$). The distillation QA
...
(Table~\ref{tab:musique_scope}). The sign reverses on each trajectory:
...
against $+0.3$). The distillation QA response predicted by the laws of
...
which only the new primary sample reproduces: the 2Wiki gain replicates
($-1.55$, $-1.26$, $-0.88$ nats), and MuSiQue and TriviaQA rise ($+0.08$ to $+0.34$).
Pruning and quantization damage agrees in sign on all three evaluation sets, as Table~\ref{tab:qa_scope} shows.
...
0.20 against 0.08 and 0.15). At $d=0.55$ the source-free curves are
...
class-indicator candidate is the least accurate, or nearly so, above 3
bits (0.25 and 0.27 against
...
state at $g=128$) is in Table~\ref{tab:quant_threeway}. Table~\ref{tab:quant_threeway} distinguishes three statuses: the
score of the frozen development-selected predictor (surface for math,
per-configuration median for code, zero for QA), the score of each
...
\caption{Paired improvements; positive values favour the first-named model. (a) Full model against its variant without $D_0$, per arm and capability, with cluster-bootstrap intervals where they were computed. (b) Source-conditioned candidate against the strongest source-free baseline on the twenty registered tests (hollow: frozen prospective; filled: leave-one-out). (c) Shared power form against the same-input alternatives A1 and A2 on the pruning configuration tests of the confirmation round.}
\label{fig:gains}
\end{figure}
...
not used in any pruning fit, at $d\in\{0.85,0.75,0.65\}$ before the
pruned losses were measured (Table~\ref{tab:prune_repeat}). The two
cached revisions labelled step 16k and step 143k carry
...
pruned losses), so the panel is one state measured twice; the
...
mirrored comparison file. Two pre-specified training-seed repeats
...
fine structure is consistent with benchmark content (SVAMP, a set of grade-school word problems,
...
Fisher signature of each capability (excluding coordinates shared with
other capabilities) and zero them. The damage matrix for Gemma-3-1B is nearly diagonal. In the table below, rows identify the ablated
...
removes one capability while leaving the others nearly unchanged
...
nothing; over 75 storage-matched pairs (14 with both sides pre-cliff) the
...
Recovery training on general text is fitted closely by the recovery law, with a recovery exponent
$\beta'_c$ per capability (Gemma3-270M pruned to $\dens=0.6$; $R^2\approx1.0$):
...
Recovery on \emph{aligned} data (teacher traces) proceeds considerably
faster at small budgets (math $2.76$ against $3.47$ at $0.5$M tokens),
...
from $0.5$M to $8$M tokens), whereas general-text recovery is monotone. Quantization damage past the bit cliff is considerably less redeemable
...
(1) the falsification anchor of that study was driven almost entirely by the
answer-only recipe (after stratification the anchor holds and the held-out
error drops from $9.0\times$ to $1.8\times$ the noise floor); (2)
same-benchmark evaluation inflated the QA gains by roughly one third;
(3) discrimination heterogeneity invalidates interval-scale ability while
rankings survive (Spearman $\rho\ge0.98$); (4) item-level damage is smooth
before the cliff, which excludes discrete skill deletion at the item level.
