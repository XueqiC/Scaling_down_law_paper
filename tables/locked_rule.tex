...
Pruning & Math & Seen source state; new density & Pruning power form & Pruning development & Source dense loss\\
\midrule
% delta = float(np.dot(p["beta"], z) * prune.shape(d, p["gamma"]))
...
Pruning & QA & Seen source state; new density & {Pruning development median curve} & Pruning development & Source dense loss\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & Math & {New size or new stage} & {Pruning development median curve} & Pruning development & Source dense loss\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & Code & {New size or new stage} & {Pruning development median curve} & Pruning development & Source dense loss\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & QA & {New size or new stage} & {Pruning development median curve} & Pruning development & Source dense loss\\
\midrule
\addlinespace
% delta = channel_delta(f["models"][c], inputs)
{Per-channel round-to-nearest quantization} & Math & Seen state & {Per-bit source regression} & Channel quantization development & Source dense loss\\
\midrule
% delta = channel_delta(f["models"][c], inputs)
...
\midrule
% delta = channel_delta(f["models"][c], inputs)
{Per-channel round-to-nearest quantization} & QA & Seen state & {Per-bit source regression} & Channel quantization development & Source dense loss\\
\midrule
% delta = f["median"][c][str(inputs["bit"])]
{Per-channel round-to-nearest quantization} & Math & New state & {Per-bit development median} & Channel quantization development & Source dense loss\\
\midrule
% delta = f["median"][c][str(inputs["bit"])]
...
\midrule
% delta = f["median"][c][str(inputs["bit"])]
{Per-channel round-to-nearest quantization} & QA & New state & {Per-bit development median} & Channel quantization development & Source dense loss\\
\midrule
\addlinespace
...
Compression method & Capability & State status & Response predictor & Development data & Dense reference loss \\
\midrule
{Grouped round-to-nearest quantization} & Math & Seen state & {Piecewise source interpolation} & Grouped quantization development & Source dense loss\\
\midrule
% anchors = {k: float(np.dot(beta, z)) for k, beta in
%                        f["models"][c]["same_input_interpolation"]["anchors"].items()}
...
{Grouped round-to-nearest quantization} & QA & Seen state & {per-configuration median} & Grouped quantization development & Source dense loss\\
\midrule
% anchors = f["models"][c]["median"]["anchors"]
{Grouped round-to-nearest quantization} & Math & New state & {per-configuration median} & Grouped quantization development & Source dense loss\\
\midrule
% anchors = f["models"][c]["median"]["anchors"]
...
\midrule
% anchors = f["models"][c]["median"]["anchors"]
{Grouped round-to-nearest quantization} & QA & New state & {per-configuration median} & Grouped quantization development & Source dense loss\\
\midrule
\addlinespace
% float(distill._predict(f["linear"][c], [{**s, "L0": anchor}])[0])
Distillation & Math & New source & {Linear source regression} & Fixed-recipe student development & Initial student dense loss\\
\midrule
% float(f["constant"][c])
Distillation & Code & New source & Constant & Fixed-recipe student development & Initial student dense loss\\
\midrule
% float(f["constant"][c])
Distillation & QA & New source & Constant & Fixed-recipe student development & Initial student dense loss\\
\midrule
\addlinespace
% value = float(inputs["L0"])
Dense & Math & Any valid status & 0 (no loss change) & --- & Source dense loss\\
\midrule
% value = float(inputs["L0"])
Dense & Code & Any valid status & 0 (no loss change) & --- & Source dense loss\\
\midrule
% value = float(inputs["L0"])
Dense & QA & Any valid status & 0 (no loss change) & --- & Source dense loss\\
\bottomrule\end{tabular*}
