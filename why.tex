...
inputs it may use. We take three questions in turn: the shape of the response along each configuration axis, the
conditions that change that shape, and the mechanism behind the damage. Section~\ref{sec:laws} then builds only on the findings that these tests establish, and states the
range of each.
...
Under pruning, the response before the cliff is a power law in the deleted capability mass, with an
exponent that transfers across families; this description uses gradient information from the dense
