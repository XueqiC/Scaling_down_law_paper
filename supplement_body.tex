...
$q_{\max}(b)=2^{b-1}-1$ the integer range at bit-width $b$.
...
\caption{Paired improvements; positive values favour the first-named model. (a) Full model against its variant without $D_0$, per arm and capability, with cluster-bootstrap intervals where they were computed. (b) Source-conditioned candidate against the strongest source-free baseline on the twenty registered tests (hollow: frozen prospective; filled: leave-one-out). (c) Shared power form against the same-input alternatives A1 and A2 on the pruning configuration tests.}
\label{fig:gains}
\end{figure}
...
\bottomrule
\end{tabular*}
\captionof*{table}{\mbox{Post-hoc} selective-ablation measurements. Rows identify the ablated capability region; columns identify the measured capability. Entries are loss increases in nats per native token; bold entries mark the matching capability on the diagonal.}
\end{center}
