...
\rev{This section describes the response structure that the predictors of Section~\ref{sec:laws} build
...
where it helps. \rev{Because the pruning shape is shared across states, a predictor can learn a state's scale
...
\caption{\rev{Compression responses on the controlled Pythia panel, the structure the predictors of Section~\ref{sec:laws} build on.} (a) Loss change against retained density on the nine development states, with the median predictions as heavy lines\rev{: mathematics and code rise along one shape with a state-dependent scale}. (b) Loss change against bit-width under per-channel round-to-nearest. (c) Loss change against group size at 3, 4 and 5 bits for one state; groups 32 and 512 were unseen when the predictions were pre-specified.}
