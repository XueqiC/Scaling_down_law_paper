...
Pruning & Math & Seen source state; new density & Pruning power form\\
\midrule
% delta = float(np.dot(p["beta"], z) * prune.shape(d, p["gamma"]))
Pruning & Code & Seen source state; new density & Pruning power form\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & QA & Seen source state; new density & {Pruning development median curve}\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & Math & {New size or new stage} & {Pruning development median curve}\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & Code & {New size or new stage} & {Pruning development median curve}\\
\midrule
% delta = prune.linear_curve(f["models"][c]["median_curve"]["anchors"], d)
Pruning & QA & {New size or new stage} & {Pruning development median curve}\\
\midrule
\addlinespace
% delta = channel_delta(f["models"][c], inputs)
{Per-channel round-to-nearest quantization} & Math & Seen state & {Per-bit source regression}\\
\midrule
% delta = channel_delta(f["models"][c], inputs)
{Per-channel round-to-nearest quantization} & Code & Seen state & {Per-bit source regression}\\
\midrule
% delta = channel_delta(f["models"][c], inputs)
{Per-channel round-to-nearest quantization} & QA & Seen state & {Per-bit source regression}\\
\midrule
% delta = f["median"][c][str(inputs["bit"])]
{Per-channel round-to-nearest quantization} & Math & New state & {Per-bit development median}\\
\midrule
% delta = f["median"][c][str(inputs["bit"])]
{Per-channel round-to-nearest quantization} & Code & New state & {Per-bit development median}\\
\midrule
% delta = f["median"][c][str(inputs["bit"])]
{Per-channel round-to-nearest quantization} & QA & New state & {Per-bit development median}\\
\midrule
\addlinespace
...
Compression method & Capability & State status & Response predictor \\
\midrule
{Grouped round-to-nearest quantization} & Math & Seen state & {Piecewise source interpolation}\\
\midrule
% anchors = {k: float(np.dot(beta, z)) for k, beta in
%                        f["models"][c]["same_input_interpolation"]["anchors"].items()}
...
{Grouped round-to-nearest quantization} & QA & Seen state & {Per-configuration development median}\\
\midrule
% anchors = f["models"][c]["median"]["anchors"]
{Grouped round-to-nearest quantization} & Math & New state & {Per-configuration development median}\\
\midrule
% anchors = f["models"][c]["median"]["anchors"]
{Grouped round-to-nearest quantization} & Code & New state & {Per-configuration development median}\\
\midrule
% anchors = f["models"][c]["median"]["anchors"]
{Grouped round-to-nearest quantization} & QA & New state & {Per-configuration development median}\\
\midrule
\addlinespace
% float(distill._predict(f["linear"][c], [{**s, "L0": anchor}])[0])
Distillation & Math & New source & {Linear source regression}\\
\midrule
% float(f["constant"][c])
Distillation & Code & New source & Frozen constant\\
\midrule
% float(f["constant"][c])
Distillation & QA & New source & Frozen constant\\
\midrule
\addlinespace
% value = float(inputs["L0"])
Dense & Math & Any valid status & 0 (no loss change)\\
\midrule
% value = float(inputs["L0"])
Dense & Code & Any valid status & 0 (no loss change)\\
\midrule
% value = float(inputs["L0"])
Dense & QA & Any valid status & 0 (no loss change)\\
\bottomrule\end{tabular*}
